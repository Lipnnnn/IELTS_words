...
def get_definition(word):
    ...
    if word_lower in definitions:
        ...
        # 返回通用释义
        return 'A word in this category'

def main():
    input_file = '../public/static/word_list.csv'
    output_file = '../public/static/word_list_new.csv'
    ...
    with open(output_file, 'w', encoding='utf-8', newline='') as f:
        fieldnames = ['word', 'meaning', 'sort', 'title', 'phonetic', 'definition']
        ...
        for row in rows:
            word = row['word']
            row['definition'] = get_definition(word)
            writer.writerow(row)
    
    print(f'成功处理 {len(rows)} 个单词')
